"""Tests for engine_weight_checksum.flatten_engine_checksums."""
...
class TestFlattenEngineChecksums:
    def test_single_server_group_engine_single_rank(self) -> None:
        ...
        assert flatten_engine_checksums(result) == [{"rank0/w": "aaa"}]
    ...
    def test_multiple_engines_flattened_in_order(self) -> None:
        ...
        assert flatten_engine_checksums(result) == [{"rank0/w": "e0"}, {"rank0/w": "e1"}]
    ...
    def test_none_node_rank_payloads_filtered(self) -> None:
        ...
        assert flatten_engine_checksums(result) == [{"rank0/w": "e0"}]
    ...
    def test_all_none_fails_loud(self) -> None:
        ...
        with pytest.raises(AssertionError, match="no non-None engine bodies"):
            flatten_engine_checksums(result)
    ...
    def test_multi_rank_merged_with_rank_prefix(self) -> None:
        ...
        assert flatten_engine_checksums(result) == [{"rank0/w": "r0", "rank1/w": "r1"}]
    ...
    def test_ranks_out_of_order_sorted_by_parallelism_rank(self) -> None:
        ...
        assert flatten_engine_checksums(out_of_order) == flatten_engine_checksums(in_order)
