"""Unit tests for resolving --use-ci-history from MILES_CI_GATE_RECORD_DIR.
...
def _load_resolver():
    ...
    wanted = {"CI_GATE_RECORD_DIR_ENV", "resolve_use_ci_history"}
    selected = []
    ...
    return namespace["resolve_use_ci_history"], namespace["CI_GATE_RECORD_DIR_ENV"]


def test_env_unset_flag_false_resolves_false():
    resolve, env_name = _load_resolver()
    assert resolve(False, env={}) is False
    # explicitly absent key, behavior unchanged
    assert resolve(False, env={"SOMETHING_ELSE": "/x"}) is False
    assert env_name == "MILES_CI_GATE_RECORD_DIR"


def test_env_set_flag_false_resolves_true():
    resolve, env_name = _load_resolver()
    assert resolve(False, env={env_name: "/some/record/dir"}) is True


def test_flag_true_resolves_true_regardless_of_env():
    resolve, env_name = _load_resolver()
    assert resolve(True, env={}) is True
    assert resolve(True, env={env_name: "/some/record/dir"}) is True


def test_empty_env_value_does_not_enable():
    resolve, env_name = _load_resolver()
    assert resolve(False, env={env_name: ""}) is False
