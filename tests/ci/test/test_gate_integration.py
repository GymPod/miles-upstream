...
from tests.ci.history_gate import compute_test_file_hash
...
class TestIsNightly:
    def test_schedule_event_is_nightly(self, monkeypatch):
        monkeypatch.setenv("GITHUB_EVENT_NAME", "schedule")
        assert is_nightly(set()) is True

    def test_nightly_label_is_nightly(self, monkeypatch):
        monkeypatch.setenv("GITHUB_EVENT_NAME", "pull_request")
        assert is_nightly({"nightly"}) is True

    def test_pr_without_label_is_not_nightly(self, monkeypatch):
        monkeypatch.setenv("GITHUB_EVENT_NAME", "pull_request")
        assert is_nightly({"megatron"}) is False
...
class TestPassingAttemptSelection:
    def test_only_passing_attempt_record_is_evaluated_and_written(self, tmp_path, store, monkeypatch):
        ...
        test_file = _write_test_file(
            tmp_path,
            'register_ci_gate(metric_key="rollout/raw_reward", hard_ref=0.80, rel=0.20)',
        )
...
class TestNightlyWrite:
    ...
    def test_nightly_writes_untrusted_when_verdict_not_trusted(self, tmp_path, store):
        ...
        test_file = _write_test_file(
            tmp_path,
            'register_ci_gate(metric_key="rollout/raw_reward", hard_ref=0.30, rel=0.20)',
        )
        registry = _registry(test_file)
...
def test_hook_signature_matches_metric_sample_contract(tmp_path, store, monkeypatch):
    ...
    # MetricSample(metric_key, sub_label, current) and only for metrics whose
    # current is not None. A spec on a missing metric (current None) yields a
    # written run with zero values.
    monkeypatch.setenv("GITHUB_EVENT_NAME", "schedule")
    test_file = _write_test_file(
        tmp_path,
        'register_ci_gate(metric_key="rollout/raw_reward", hard_ref=0.80, rel=0.20)',
    ...
    assert MetricSample("k", None, 1.0).value == 1.0
