"""Phase-1 OpenEnv smoke launcher (GLM-4.7-Flash, colocated single-node).

Derived from ``swe-agent-v2/run.py`` with the Harbor agent server removed: the
agent loop runs in-process (``openenv_echo_agent_function.run``) and drives a
HuggingFace OpenEnv environment (hosted Echo by default) instead of POSTing to a
Harbor server.

Purpose: validate the miles <-> OpenEnv integration *seam* end to end -- that the
policy's tokens are captured (via the session server) and the env reward flows
into the GRPO sample. Echo's reward is trivial, so this proves plumbing, NOT
learning.

Prereqs in the training image:
    pip install openai
    pip install git+https://huggingface.co/spaces/openenv/echo_env
    python make_echo_data.py --output /root/echo_train.jsonl --n 64

Usage:
    python run-openenv-echo.py
    python run-openenv-echo.py --openenv-env-url http://localhost:8001  # local Docker env
"""
...
class ScriptArgs(U.ExecuteTrainConfig):
    ...
    # OpenEnv settings
    openenv_env_url: str = os.environ.get("OPENENV_ENV_URL", "https://openenv-echo-env.hf.space")
    ...
    wandb_project: str = os.environ.get("WANDB_PROJECT", "openenv-echo-smoke")
    wandb_team: str = os.environ.get("WANDB_TEAM", "")
    wandb_run_name: str = "openenv-echo-smoke"

    # Prometheus settings
    use_prometheus: bool = True
    prometheus_port: int = 9090
    prometheus_run_name: str = "openenv-echo-smoke"
