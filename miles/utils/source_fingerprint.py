...
def _numpy_extended_scalar_digest(value: numpy.generic) -> bytes:
    if value.dtype.kind == "f":
        formatted = numpy.format_float_scientific(value, unique=True, trim="k")
        return _digest_parts(b"extended-float", formatted.encode())
    if value.dtype.kind == "c":
        formatted_real = numpy.format_float_scientific(numpy.real(value), unique=True, trim="k")
        formatted_imag = numpy.format_float_scientific(numpy.imag(value), unique=True, trim="k")
...
def canonical_source_digest(value: object) -> bytes:
    ...
    if isinstance(value, Image.Image):
        return _digest_parts(
            ...
            canonical_source_digest(value.info.get("transparency")),
        )
    ...
    raise TypeError(
        f"Cannot fingerprint rollout source value of type "
