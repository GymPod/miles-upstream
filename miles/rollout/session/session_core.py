"""HTTP core of the session server.

Holds the session / TITO request logic over primitive inputs (method, path,
query string, headers, body bytes) and returns Starlette ``Response`` objects.
The single-process FastAPI adapter (``sessions.py`` + ``session_server.py``)
wires HTTP requests to these methods.

This module is a structural extraction of the previous single-process route
handlers — the observable behavior (status codes, parsed response content,
error shapes, recorded trajectory) is unchanged.
"""
