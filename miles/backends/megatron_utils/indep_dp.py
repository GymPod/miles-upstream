...
def reconfigure_indep_dp_group(
    ...
    """Tear down old indep_dp PGs and create new ones with a fresh quorum_id."""
    old = parallel_state.indep_dp
    for g in [old.group, old.gloo_group]:
        if g is not None:
            # Tear down via torchft's abort() (ncclCommAbort), the same path
            # torchft's own configure() uses to reconfigure across quorums. NOT
            # shutdown(): shutdown only drops the ref and runs the blocking
            # ~ProcessGroupNCCL destructor (abort()+waitForFutureOrTimeout),
            # which hangs forever when the peer cell died mid-collective.
