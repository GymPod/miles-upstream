---
name: doc-dev
description: "Keep a file consistent with its governing doc(s). A file opts in via a `# doc-dev:` sentinel comment or by explicit invocation. Any `# doc-dev:` marker binds the file's own header doc (its function, responsibility boundary, inputs/outputs, and general design) — consistency means the header still describes the code. A `# doc-dev: <path>` marker adds a central requirement/target doc — consistency means the code conforms to it. Co-evolution is the default (build the code, then update the doc); pass `--docfirst` for doc-first (the doc is the authority and the code conforms, stopping to ask before rewriting a contract). Opt-in only — a file that carries no sentinel and is not explicitly targeted is untouched. Use when editing a `# doc-dev:`-flagged file, when editing a central doc that flagged code names, or when conforming code to a doc you declare authoritative."
user_invocable: true
---
